...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 1. Groupe Membres
        group_members, created = Group.objects.get_or_create(name='Membres')
        ...
        self.stdout.write(self.style.SUCCESS(f"Groupe 'Membres' provisionné ({member_perms.count()} permissions)."))

        # 2. Groupe Administrateurs
        group_admins, created = Group.objects.get_or_create(name='Administrateurs')
        ...
        self.stdout.write(self.style.SUCCESS(f"Groupe 'Administrateurs' provisionné ({admin_perms.count()} permissions)."))

        # 3. Groupe Super-Administrateurs
        group_superadmins, created = Group.objects.get_or_create(name='Super-Administrateurs')
